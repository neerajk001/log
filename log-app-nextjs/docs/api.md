...
Base URL: same origin — `/api` (the Next.js app serves the UI and API from
one domain). All routes except `/api/health` require a valid Clerk session
(enforced by `clerkMiddleware` + `auth()` in the handler — see
`docs/auth.md`). Because the browser and API are same-origin, the session
cookie is sent automatically; no manual `Authorization` header is needed
from the web client.
...
## Route Handler implementation note

Each endpoint is a Route Handler file under `app/api/**/route.ts` exporting
`GET`/`POST`/`PUT` functions. Dynamic segments use the folder name, e.g.
`app/api/logs/daily/[date]/route.ts` exports `GET(req, { params })` and
`PUT(req, { params })`. (In Next.js 15, `params` is a Promise — `await
params`.)

## `GET /api/health`

No auth. Returns `{status: "ok"}`. Used for platform health checks.
...
Calls OpenAI server-side (see `backend.md` for the prompt).
