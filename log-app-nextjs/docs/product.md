...
"Log" is a minimal, honest fitness-tracking web app for people in a structured
fat-loss or recomposition phase who are already training with a plan. It
replaces manual coaching judgment calls ("should I hold or adjust?") with
three simple daily logs and one weekly automated verdict.

> **Platform note:** This is the **Next.js fullstack** re-plan. The original
> product brief specified a native mobile app and listed "Web app" as out of
> scope. This build delivers the same product as a **mobile-only web app**
> — it runs in the mobile browser only, with a mobile-first, single-column
> layout and no desktop layout (see `docs/AGENTS.md` for the scope flag and
> `docs/design-system.md` for the mobile-width frame). All product behavior,
> features, and the design philosophy below are unchanged.
...
> Note: "Web app (mobile-only)" from the original brief is intentionally
> overridden by this Next.js build (see `docs/AGENTS.md`).
