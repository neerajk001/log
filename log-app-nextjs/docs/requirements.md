...
considered done. Reference the phase files in `docs/` for build order.
These are identical in substance to the original spec; only the rendering
platform differs (web, not native).

## R1 — Authentication

- R1.1 User can sign up with email via Clerk.
- R1.2 User can log in / stay logged in across page reloads and restarts.
...
**Acceptance:** A new user can sign up, close the tab, reopen it, and land
...
- R4.2 Backend sends the text to OpenAI with a fixed prompt/schema
